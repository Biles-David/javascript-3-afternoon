...
//Code Here
let myStrongest = monstersInYourPocket.filter(elm => elm.CP > 200)
...
let orderTotals = orders.map(num => num.price * (1 + num.tax))
// Code here
...
let bobsTotal = purchases.filter(elm => elm.owner === 'Bob').map(num=>num.price).reduce((total,num)=>total+num)
